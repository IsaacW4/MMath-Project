...
#must rewrite
try:
    RESOLUTION = list(map(lambda x:int(x),cfp.get('lofi','Resolution').split(',')))
...
if not LOFI:
    try:
        RESOLUTION = list(map(lambda x:int(x),cfp.get('hifi','Resolution').split(',')))
...
try:
    CAMERA_POS = np.array(list(map(lambda x:float(x),cfp.get('geometry','Cameraposition').split(','))))
    TANFOV = float(cfp.get('geometry','Fieldofview'))
    LOOKAT = np.array(list(map(lambda x:float(x),cfp.get('geometry','Lookat').split(','))))
    UPVEC = np.array(list(map(lambda x:float(x),cfp.get('geometry','Upvector').split(','))))
...
if SKY_TEXTURE == 'texture':
    ...
    texarr_rows = texarr_sky.shape[0]
    texarr_cols = texarr_sky.shape[1]
    ...
    if not LOFI:
        ...
        texarr_sky = resize(texarr_sky,(texarr_rows, texarr_cols))
...
raytrace_schedule(0, schedules[0], total_colour_buffer_preproc_shared, output.queue)
